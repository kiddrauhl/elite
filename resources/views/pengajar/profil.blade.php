...
<div class="p-6 space-y-6 max-w-3xl mx-auto">
    ...
    <div>
        ...
        <p class="text-sm text-slate-500 mt-1">Perbarui informasi dasar, foto profil, dan keamanan akun Anda.</p>
    </div>
    ...
    <div class="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
        <form action="{{ route('pengajar.profil.update') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-6">
                ...
                <div class="flex items-center gap-4">
                    @if($profil->foto_profil)
                        <img src="{{ asset('storage/' . $profil->foto_profil) }}" class="w-16 h-16 rounded-full object-cover border-4 border-slate-50 shadow-sm">
                    @else
                        <div class="w-16 h-16 rounded-full bg-slate-100 flex items-center justify-center text-slate-400 border-4 border-slate-50 shadow-sm">
                            <i class="fa-solid fa-user text-2xl"></i>
                        </div>
                    @endif
                    <div>
                        <input type="file" name="foto_profil" accept="image/*" class="block w-full text-sm text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100 cursor-pointer">
            ...
            <div class="flex justify-end pt-4 border-t border-slate-100">
                <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2.5 px-8 rounded-lg transition-colors flex items-center justify-center gap-2 shadow-sm w-full sm:w-auto">
